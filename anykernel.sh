...
# =============
# Cài đặt tự động APK App Quản Lý
# =============
APK_FILE=$(ls $AKHOME/*.apk 2>/dev/null | head -n 1)
if [ -f "$APK_FILE" ]; then
        ...
        "KEY_VOLUMEDOWN")
            ...
            if [ "$IS_BOOTED" = true ]; then
                ...
                if [ $? -eq 0 ]; then
                    ...
                    ui_print " ❌ Lỗi hệ thống khi cài. Đã chép file ra bộ nhớ trong."
                    cp "$APK_FILE" "/sdcard/Download/$APK_NAME"
                    ui_print " Vui lòng tìm file trong thư mục Download để cài thủ công."
                fi
            else
                # Môi trường Recovery (TWRP / OrangeFox)
                ui_print " ⚠️ Bạn đang ở chế độ Recovery, không thể tự động cài."
                cp "$APK_FILE" "/sdcard/Download/$APK_NAME"
                ui_print " ✅ Đã chép file APK vào thư mục /sdcard/Download/"
                ui_print " Vui lòng vào ứng dụng Quản lý Tệp cài thủ công sau khi khởi động máy."
            fi
            ;;
        "KEY_VOLUMEUP")
            ui_print " Đã bỏ qua cài đặt ứng dụng."
            ;;
        *)
            ui_print " Phím bấm không hợp lệ. Bỏ qua cài đặt."
            ;;
    esac
fi